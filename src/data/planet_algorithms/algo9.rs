    ...
    fn generate_terrain(&self, planet: &Planet, planet_raw_data: &PlanetRawData) -> Vec<u16> {
        ...
        for i in 0..data_length {
            ...
            let layer3_noise = noise1.noise_3d_fbm(
                world_x * freq_scale_x * mod_x,
                world_y * freq_scale_y * mod_x,
                world_z * freq_scale_z * mod_x,
                6,
                0.5,
                2.0,
            ) * noise_amplitude
            ...
            let blend_factor = if stretched_height >= -mod_y {
                0.0
            } else {
                (((stretched_height + mod_y).abs() / 5.0).min(1.0)).powf(1.0)
            };

            let blended_height =
                stretched_height * (1.0 - blend_factor) + alt_height * blend_factor;
