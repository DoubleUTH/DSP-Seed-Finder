...
impl<'a> Star<'a> {
                ...
                match self.star_type {
                    ...
                    _ => {
                        let num8 = if spectr_factor != 0.0 {
                            ...
                            let num7 = lerp(-0.98, 0.88, self.level);
    ...
    pub fn get_max_hive_count(&self) -> i32 {
        *self.max_hive_count.get_or_init(|| {
            let num14 = match self.star_type {
                StarType::BlackHole | StarType::NeutronStar => 2.0,
                _ => 1.0,
            };
            ((self.game_desc.hive_max_density * num14 * 1000.0
    ...
    pub fn get_initial_hive_count(&self) -> i32 {
        *self.initial_hive_count.get_or_init(|| {
            ...
            if self.is_birth() {
                let num8 = if initial_colonize * (max_hive_count as f64) < 0.7 {
                    0
                } else {
                    1
                };
                let mut num9 = 0.6 * (initial_colonize as f32) * (max_hive_count as f32);
                let mut sd = 0.5;
                if num9 < 1.0 {
                    sd = ((num9.sqrt() as f64) * 0.29 + 0.21) as f32;
                } else if num9 > max_hive_count as f32 {
                    num9 = max_hive_count as f32;
                }
                ...
                for _ in 0..17 {
                    ...
                    initial_hive_count = (rand_normal(num9, sd, r1_2, r2_2) + 0.5) as i32;
                    if initial_hive_count >= 0 && initial_hive_count <= max_hive_count {
                        break;
                    }
                }
                return initial_hive_count.clamp(num8, max_hive_count);
            }
            let a = ((1.0
                - (((self.get_safety_factor() * 1.05 - 0.15) as f32)
                    .clamp(0.0, 1.0)
                    .powf(0.82) as f64)
                - (((max_hive_count - 1) as f64) * 0.05)) as f32)
                .clamp(0.0, 1.0)
                * ((1.1 - (max_hive_count as f64) * 0.1) as f32);
            let num15a = if initial_colonize > 1.0 {
                lerp(
                    a,
                ...
                a * (initial_colonize as f32)
            };
            let num15 = match self.star_type {
                StarType::GiantStar => num15a * 1.2,
                StarType::WhiteDwarf => num15a * 1.4,
                StarType::NeutronStar => num15a * 1.6,
                StarType::BlackHole => num15a * 1.8,
                _ => {
            ...
            let num16: f32 = (num15 * (max_hive_count as f32)).min((max_hive_count as f32) + 0.75);
            let sd2: f32 = if num16 <= 0.01 {
                0.0
            } else if num16 < 1.0 {
                num16.sqrt() * 2.9 + 2.1
            } else if num16 > 1.0 {
            ...
            for _ in 0..65 {
                let r1_2 = rand3.next_f64();
                let r2_2 = rand3.next_f64();
                initial_hive_count = (rand_normal(num16, sd2, r1_2, r2_2) + 0.5) as i32;
                if initial_hive_count >= 0 && initial_hive_count <= max_hive_count {
                    break;
                }
            }
